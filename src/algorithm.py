...
---------------------------------
This module implements prefix sums with O(log n) updates/queries.
...
class BitResult:
    ...
    def __repr__(self) -> str:
        return f"<BitResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> BitResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (prefix sums with O(log n) updates/queries).
    kwargs : Any
    ...
    return BitResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: BitResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Fenwick Tree BIT: acc={acc:.4f}, trace_points={pts}"
